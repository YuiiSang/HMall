...
//    private final DiscoveryClient discoveryClient;
    ...
    private void handleCartItems(List<CartVO> vos) {
        ...
        // 1.获取商品id
        Set<Long> itemIds = vos.stream().map(CartVO::getItemId).collect(toSet());
        // 2.查询商品
//        Object itemService;
...
//
//        String url = serviceInstance.getUri().toString() + "/items?ids={ids}";
//        System.out.println("99999999999"+url);
...
//                },
//                Map.of("ids", String.join(",", itemIds.stream()
//                        .map(String::valueOf)
//                        .collect(joining(","))))
//        );
...
//        List<ItemDTO> items = ids.getBody();
        ...
        // 3.转为 id 到 item的map
        Map<Long, ItemDTO> itemMap = items.stream().collect(toMap(ItemDTO::getId, Function.identity()));
        // 4.写入vo
        for (CartVO v : vos) {
